...
if [ -z $PHP_FPM_SERVER ]; then
    FPM_SERVER="unix:/run/php/php5.6-fpm.sock;"
else
...
if [ ! -z $TIMEZONE ] && [ -f /etc/php/5.6/fpm/php.ini ] && [ -f /etc/php/5.6/cli/php.ini ]; then
    sed -i "s/date.timezone =.*/date.timezone = $TIMEZONE/" /etc/php/5.6/fpm/php.ini
    sed -i "s/date.timezone =.*/date.timezone = $TIMEZONE/" /etc/php/5.6/cli/php.ini
    echo $TIMEZONE > /etc/timezone && dpkg-reconfigure -f noninteractive tzdata
fi

# Start PHP and NGINX
php-fpm5.6 -R && nginx -g 'daemon off;'
